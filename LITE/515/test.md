...
师资队伍要搞成分布式节点。码农能来教逻辑架构，家庭主妇可以讲资源调度实战。家长每周得提交PR（参与教学），搞土木的爹带学生测绘校园地形，开花店的妈教色彩参数搭配。整个社区都是扩展包，超市变成供需模型实验室，消防站加载安全教育模组。

这套系统必须支持多线程运行。学习速率快的开性能模式，需要缓冲的给足加载时间。特殊需求用户直接内嵌辅助功能，手语和盲文作为可选插件。毕业不搞统一认证，可以是提交发明专利，可以是完成长篇xiao，有人点亮三项生存技能树。
